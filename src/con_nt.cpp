...
int ConGetEvent(TEventMask EventMask, TEvent *Event, int WaitTime, int Delete) /*FOLD00*/
{
    ...
    {
        if (Pipes[i].used) o_ar[nh++] = Pipes[i].NewData;
    }
...
//      if((rc != WAIT_FAILED) && (rc >= WAIT_OBJECT_0 && rc < WAIT_OBJECT_0+nh))
        if((rc != WAIT_FAILED) && (rc > WAIT_OBJECT_0 && rc < WAIT_OBJECT_0+nh))
        {
            ...
            if(i == 0) if(ReadConsoleEvent(Event)) return 0; // Was console? Get console, And exit if valid,
            else
            {
        ...
        else return -1;                                      // Something's wrong!
    }
}
