...
MRESULT EXPENTRY FileDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
    ...
    {
    case WM_INITDLG:
         WinSendMsg(hwnd, WM_SETICON, MPFROMLONG(WinLoadPointer(HWND_DESKTOP, 0, 1)), 0);
...
// File Open and Save as dialog... why he was using a custom dialog???
int DLGGetFile(GView *View, const char *Prompt, unsigned int BufLen, char *FileName, int Flags)
{
    ...
    if (!LONGFROMMR(WinSendMsg(View->Parent->Peer->hwndFrame,
                               UWM_FILEDIALOG, MPFROMP(&dlg), 0))) return 0;
...
//    HWND hwndStatic;
    ...
    WinSetWindowPos(hwndDlg, 0, x, y, nx, ny, SWP_SIZE | SWP_SHOW | SWP_MOVE | SWP_ACTIVATE);

    WinSubclassWindow(hwndDlg, (PFNWP) WinDefDlgProc);

    if (choice->Flags & (GPC_ERROR | GPC_FATAL)) WinAlarm(HWND_DESKTOP, WA_ERROR);
    else if (choice->Flags & (GPC_CONFIRM)) WinAlarm(HWND_DESKTOP, WA_NOTE);
         else if (choice->Flags & (GPC_WARNING)) WinAlarm(HWND_DESKTOP, WA_WARNING);

    rc = LONGFROMMR(WinSendMsg(hwndFrame, UWM_PROCESSDLG, MPFROMLONG(hwndDlg), 0));

    WinStoreWindowPos("FTEPM", msgbox, hwndDlg);

    WinSendMsg(hwndFrame, UWM_DESTROYHWND, MPFROMLONG(hwndDlg), 0);

    if (rc == DID_CANCEL || rc == DID_ERROR) return choice->NSel - 1;

    if (rc >= 200 && rc < choice->NSel + 200) return rc - 200;
...
static void _LNK_CONV WorkThread(void *)
{
    ...
    if (gui->Start(gui->fArgc, gui->fArgv) == 0)
    {
        gui->doLoop = 1;
        //DosBeep(500, 500);
        while (gui->doLoop) gui->ProcessEvent();
...
int GViewPeer::ConPutLine(int X, int Y, int W, int H, PCell Cell) {
    ...
    for (I = 0; I < H; I++)
    {
        if (I + Y == cY) Hidden = PMHideCursor();
        VioWrtCellStr(p, (USHORT)(W << 1), (USHORT)(Y + I), (USHORT)X, pmData->hvps);
        if (Hidden) PMShowCursor();
    }
    DosReleaseMutexSem(hmtxPMData);
    return 0;
}
...
GView::~GView()
{
    if (FocusCapture == this) CaptureFocus(0);
    if (MouseCapture == this) CaptureMouse(0);
...
{
    if (Peer->ConSetSize(X, Y)) Resize(X, Y);
    else return 0;
...
int GView::Execute()
{
    ...
    else if (FocusCapture != this) return -1;
    Result = -2;
    while (Result == -2 && frames != 0) gui->ProcessEvent();
    NewResult = Result;
    Result = SaveRc;
    if (didFocus) CaptureFocus(0);
    return NewResult;
}
...
int GView::CaptureMouse(int grab)
{
    ...
    {
        if (grab || MouseCapture != this) return 0;
        else
        {
            MouseCapture = 0;
...
int GView::CaptureFocus(int grab)
{
    ...
    {
        if (grab || FocusCapture != this) return 0;
        else FocusCapture = 0;
    }
    return 1;
}
...
{
    if (Active != 0) return ConSplitView(Active, view);
    else
    {
//        int W, H;
...
void GFrame::RemoveView(GView *view)
{
    ...
    if (Active == view) Active->Activate(0);
    if (view->Next == view)
    {
        Top = Active = 0;
        delete this;
...
int GUI::ConGrabEvents(TEventMask EventMask) { return 0; }

void GUI::DispatchEvent(GFrame *frame, GView *view, TEvent &Event)
{
    if (Event.What != evNone) if (view) view->HandleEvent(Event);
}
...
int GUI::Run()
{
    ...
    while (doLoop != 0 && WinGetMsg(hab, &qmsg, NULLHANDLE, 0, 0)) WinDispatchMsg(hab, &qmsg);
    return 0;
}

int GUI::ShowEntryScreen() { return 1; }
...
static int CreatePipeChild(ULONG *sid, PID *pid, HPIPE &hfPipe, char *Command)
{
    ...
    //ULONG ulNew;
    HPIPE hfChildPipe;
    HFILE hfNewStdOut = (HFILE)-1;
    HFILE hfNewStdErr = (HFILE)-1;
    HFILE hfStdOut = (HFILE)1;
    HFILE hfStdErr = (HFILE)2;
    int rc;
...
#if 0
    Args = (char *)malloc(strlen(Prog) + 1 + 3 + strlen(Command) + 1 + 1);
...
static void PipeThread(void *p)
{
    ...
    if (rc != 0)
    {
        ...
        pipe->reading = 0;
        if (pipe->notify) WinPostMsg(frames->Active->Peer->hwndWorker, UWM_NOTIFY, MPFROMLONG(pipe->notify), MPFROMLONG(pipe->id));
...
int GUI::OpenPipe(char *Command, EModel *notify)
{
    ...
    for (i = 0; i < MAX_PIPES; i++)
    {
        if (Pipes[i].used == 0)
        {
            ...
            Pipes[i].DoTerm = 0;
            if ((Pipes[i].buffer = (char *)malloc(PIPE_BUFLEN)) == 0) return -1;
...
//    fprintf(stderr, "Pipe Read: Get %d %d\n", id, len);
    ...
    {
        l = len;
        if (l > Pipes[id].bufused - Pipes[id].bufpos) l = Pipes[id].bufused - Pipes[id].bufpos;

        memcpy(buffer, Pipes[id].buffer + Pipes[id].bufpos, l);
        Pipes[id].bufpos += l;
...
//            fprintf(stderr, "Pipe Resume Read: %d\n", id);
    ...
    else if (Pipes[id].reading == 0) l = -1;
         else l = 0;  //DosBeep(200, 200);
